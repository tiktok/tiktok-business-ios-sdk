...
// files, arranged for use in TTSDKObjC.

#ifndef HDR_TTSDKObjCApple_h
#define HDR_TTSDKObjCApple_h
...
#if OBJC_HAVE_TAGGED_POINTERS // (TTSDKCrash) This line is moved here to make `objc_tag_index_t` enum visible for i386.
...
// (TTSDKCrash) Added to handle runtime checks for Split Tagged Pointers,
...
static inline bool
ttsdkc_objc_splitTaggedPointersEnabled(void);
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 8 : 16;
}

static inline uintptr_t
_OBJC_TAG_SLOT_MASK_func(void)
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 0x7UL : 0xfUL;
}

static inline uintptr_t
_OBJC_TAG_MASK_func(void)
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? (1UL<<63) :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 0 :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 1 :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 4 :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? (_OBJC_TAG_MASK_func() | 0x7UL) :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? ((1UL<<62) | _OBJC_TAG_EXT_MASK_func()) : 0;
}

static inline int
_OBJC_TAG_EXT_SLOT_SHIFT_func(void)
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 55 :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ?
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 9 :
...
{
    return ttsdkc_objc_splitTaggedPointersEnabled() ? 12 :
        OBJC_MSB_TAGGED_POINTERS ? 12 : 12;
}

// (TTSDKCrash) Macros were rewritten to include runtime checks for split tagged pointers, based on objc4-912.3.
...
// (TTSDKCrash) Weakly linked to support *OS versions prior to objc4-818.2, where this symbol may not be available.
...
static inline uintptr_t _objc_basicTagToObfuscatedTag(uintptr_t tag) {
    ...
    // (TTSDKCrash) Fallback: return the original tag if permutations are unavailable or tag is out of range.
...
static inline uintptr_t _objc_obfuscatedTagToBasicTag(uintptr_t tag) {
    ...
    // (TTSDKCrash) Fallback: return the original tag if within range, otherwise return 7.
    // Runtime handles the availability of split tagged pointers, so fallback is typically unnecessary,
    // but it's included as a safeguard.
    return (tag < 8) ? tag : 7;
}
...
#if OBJC_SPLIT_TAGGED_POINTERS
    if (ttsdkc_objc_splitTaggedPointersEnabled()) {
