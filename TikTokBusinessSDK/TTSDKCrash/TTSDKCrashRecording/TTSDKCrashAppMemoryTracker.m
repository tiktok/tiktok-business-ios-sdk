...
static TTSDKCrashAppMemoryProvider gMemoryProvider = nil;
...
@interface TTSDKCrashAppMemoryTracker () {
    ...
    uint64_t _footprint;
    TTSDKCrashAppMemoryState _pressure;
    TTSDKCrashAppMemoryState _level;
}
@end

@implementation TTSDKCrashAppMemoryTracker

- (instancetype)init
{
    if (self = [super init]) {
        _lock = OS_UNFAIR_LOCK_INIT;
        _heartbeatQueue = dispatch_queue_create_with_target("com.TTSDKCrash.memory.heartbeat", DISPATCH_QUEUE_SERIAL,
                                                            dispatch_get_global_queue(QOS_CLASS_DEFAULT, 0));
        _level = TTSDKCrashAppMemoryStateNormal;
        _pressure = TTSDKCrashAppMemoryStateNormal;
    }
    return self;
}
...
- (void)start
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(_appDidFinishLaunching)
                                               ...
                                               object:nil];
#endif
    [self _handleMemoryChange:[self currentAppMemory] type:TTSDKCrashAppMemoryTrackerChangeTypeNone];
}
...
static TTSDKCrashAppMemory *_Nullable _ProvideCrashAppMemory(TTSDKCrashAppMemoryState pressure)
{
    ...
    return [[TTSDKCrashAppMemory alloc] initWithFootprint:info.phys_footprint remaining:remaining pressure:pressure];
}

- (nullable TTSDKCrashAppMemory *)currentAppMemory
{
    return gMemoryProvider ? gMemoryProvider() : _ProvideCrashAppMemory(_pressure);
}

- (void)_handleMemoryChange:(TTSDKCrashAppMemory *)memory type:(TTSDKCrashAppMemoryTrackerChangeType)changes
{
...
// ie: MAX(x,y) - MIN(x,y)
#define _TTSDKABS_DIFF(x, y) x > y ? x - y : y - x

- (void)_heartbeat:(BOOL)sendObservers
{
    // This handles the memory limit.
    TTSDKCrashAppMemory *memory = [self currentAppMemory];

    TTSDKCrashAppMemoryState newLevel = memory.level;
    uint64_t newFootprint = memory.footprint;

    TTSDKCrashAppMemoryState oldLevel;
    BOOL footprintChanged = NO;
    {
        ...
        const uint64_t kTTSDKCrashFootprintMinChange = 1 << 20;  // 1 MiB
        ...
        if (_TTSDKABS_DIFF(newFootprint, _footprint) > kTTSDKCrashFootprintMinChange) {
    ...
    TTSDKCrashAppMemoryTrackerChangeType changes =
        (newLevel != oldLevel) ? TTSDKCrashAppMemoryTrackerChangeTypeLevel : TTSDKCrashAppMemoryTrackerChangeTypeNone;

    if (footprintChanged) {
        changes |= TTSDKCrashAppMemoryTrackerChangeTypeFootprint;
    }

    if (changes != TTSDKCrashAppMemoryTrackerChangeTypeNone) {
        [self _handleMemoryChange:memory type:changes];
    }

    if (newLevel != oldLevel && sendObservers) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[NSNotificationCenter defaultCenter] postNotificationName:TTSDKCrashAppMemoryLevelChangedNotification
                                                                object:self
                                                              userInfo:@{
                                                                  TTSDKCrashAppMemoryNewValueKey : @(newLevel),
                                                                  TTSDKCrashAppMemoryOldValueKey : @(oldLevel)
                                                              }];
        });
#if TARGET_OS_SIMULATOR
        ...
        dispatch_once(&onceToken, ^{
            ...
            sSimulatorMemoryKillEnabled = [env[@"TTSDKCRASH_SIM_MEMORY_TERMINATION_ENABLED"] boolValue];
        });
        if (sSimulatorMemoryKillEnabled && !sIsRunningInTests && newLevel == TTSDKCrashAppMemoryStateTerminal) {
            kill(getpid(), SIGKILL);
            _exit(0);
        }
#endif
...
- (void)_memoryPressureChanged:(BOOL)sendObservers
{
    ...
    TTSDKCrashAppMemoryState newPressure = TTSDKCrashAppMemoryStateNormal;
    dispatch_source_memorypressure_flags_t flags = dispatch_source_get_data(_pressureSource);
    switch (flags) {
        case DISPATCH_MEMORYPRESSURE_NORMAL:
            newPressure = TTSDKCrashAppMemoryStateNormal;
            break;

        case DISPATCH_MEMORYPRESSURE_WARN:
            newPressure = TTSDKCrashAppMemoryStateWarn;
            break;

        case DISPATCH_MEMORYPRESSURE_CRITICAL:
            newPressure = TTSDKCrashAppMemoryStateCritical;
            break;
    }

    TTSDKCrashAppMemoryState oldPressure;
    {
        os_unfair_lock_lock(&_lock);
    ...
    if (oldPressure != newPressure && sendObservers) {
        [self _handleMemoryChange:[self currentAppMemory] type:TTSDKCrashAppMemoryTrackerChangeTypePressure];
        [[NSNotificationCenter defaultCenter] postNotificationName:TTSDKCrashAppMemoryPressureChangedNotification
                                                            object:self
                                                          userInfo:@{
                                                              TTSDKCrashAppMemoryNewValueKey : @(newPressure),
                                                              TTSDKCrashAppMemoryOldValueKey : @(oldPressure)
                                                          }];
    }
}

- (TTSDKCrashAppMemoryState)pressure
{
    TTSDKCrashAppMemoryState state;
    {
        os_unfair_lock_lock(&_lock);
        state = _pressure;
        os_unfair_lock_unlock(&_lock);
