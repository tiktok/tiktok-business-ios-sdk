...
@interface TTSDKCrashDoctorParam : NSObject
...
@implementation TTSDKCrashDoctorParam

@end

@interface TTSDKCrashDoctorFunctionCall : NSObject
...
@implementation TTSDKCrashDoctorFunctionCall
...
- (NSString *)descriptionForObjCCall
{
    ...
    TTSDKCrashDoctorParam *receiverParam = [self.params objectAtIndex:0];
    ...
    TTSDKCrashDoctorParam *selectorParam = [self.params objectAtIndex:1];
    if (![selectorParam.type isEqualToString:TTSDKCrashMemType_String]) {
        return nil;
    }
    ...
    for (int paramNum = 0; paramNum < paramCount; paramNum++) {
        [string appendString:@":"];
        if (paramNum < 2) {
            TTSDKCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if (param.value != nil) {
                if ([param.type isEqualToString:TTSDKCrashMemType_String]) {
...
- (NSString *)descriptionWithParamCount:(int)paramCount
{
    ...
    for (int i = 0; i < paramCount; i++) {
        TTSDKCrashDoctorParam *param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation TTSDKCrashDoctor
...
{
    return [[self crashReport:report] objectForKey:TTSDKCrashField_Error];
}
...
- (NSDictionary *)crashedThreadReport:(NSDictionary *)report
{
    ...
    NSDictionary *crashedThread = [crashReport objectForKey:TTSDKCrashField_CrashedThread];
    if (crashedThread != nil) {
        return crashedThread;
    }

    for (NSDictionary *thread in [crashReport objectForKey:TTSDKCrashField_Threads]) {
        if ([[thread objectForKey:TTSDKCrashField_Crashed] boolValue]) {
            return thread;
        }
    }
    return nil;
...
{
    NSDictionary *backtrace = [threadReport objectForKey:TTSDKCrashField_Backtrace];
    return [backtrace objectForKey:TTSDKCrashField_Contents];
}

- (NSDictionary *)basicRegistersFromThreadReport:(NSDictionary *)threadReport
{
    NSDictionary *registers = [threadReport objectForKey:TTSDKCrashField_Registers];
    NSDictionary *basic = [registers objectForKey:TTSDKCrashField_Basic];
    return basic;
}
...
- (NSDictionary *)lastInAppStackEntry:(NSDictionary *)report
{
    ...
    for (NSDictionary *entry in backtrace) {
        NSString *objectName = [entry objectForKey:TTSDKCrashField_ObjectName];
...
{
    NSDictionary *machError = [errorReport objectForKey:TTSDKCrashField_Mach];
    if (machError != nil) {
        NSString *exceptionName = [machError objectForKey:TTSDKCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary *signal = [errorReport objectForKey:TTSDKCrashField_Signal];
    NSString *sigName = [signal objectForKey:TTSDKCrashField_Name];
...
{
    NSDictionary *crashedThread = [self crashedThreadReport:report];
    NSArray *notableAddresses = [crashedThread objectForKey:TTSDKCrashField_NotableAddresses];
    for (NSDictionary *address in [notableAddresses objectEnumerator]) {
        NSString *type = [address objectForKey:TTSDKCrashField_Type];
        if ([type isEqualToString:@"string"]) {
            NSString *value = [address objectForKey:TTSDKCrashField_Value];
    ...
    for (NSDictionary *entry in backtrace) {
        NSString *objectName = [entry objectForKey:TTSDKCrashField_ObjectName];
        NSString *symbolName = [entry objectForKey:TTSDKCrashField_SymbolName];
...
- (TTSDKCrashDoctorFunctionCall *)lastFunctionCall:(NSDictionary *)report
{
    TTSDKCrashDoctorFunctionCall *function = [[TTSDKCrashDoctorFunctionCall alloc] init];
    NSDictionary *lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:TTSDKCrashField_SymbolName];
    ...
    for (NSString *regName in regNames) {
        TTSDKCrashDoctorParam *param = [[TTSDKCrashDoctorParam alloc] init];
        ...
        if (notableAddress == nil) {
            ...
            param.type = [notableAddress objectForKey:TTSDKCrashField_Type];
            NSString *className = [notableAddress objectForKey:TTSDKCrashField_Class];
            NSString *previousClass = [notableAddress objectForKey:TTSDKCrashField_LastDeallocObject];
            NSString *value = [notableAddress objectForKey:TTSDKCrashField_Value];

            if ([param.type isEqualToString:TTSDKCrashMemType_String]) {
                param.value = value;
            } else if ([param.type isEqualToString:TTSDKCrashMemType_Object]) {
                param.className = className;
                param.isInstance = YES;
            } else if ([param.type isEqualToString:TTSDKCrashMemType_Class]) {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString *)zombieCall:(TTSDKCrashDoctorFunctionCall *)functionCall
{
...
{
    NSDictionary *stack = [crashedThreadReport objectForKey:TTSDKCrashField_Stack];
    return [[stack objectForKey:TTSDKCrashField_Overflow] boolValue];
}

- (BOOL)isDeadlock:(NSDictionary *)report
{
    NSDictionary *errorReport = [self errorReport:report];
    NSString *crashType = [errorReport objectForKey:TTSDKCrashField_Type];
    return [TTSDKCrashExcType_Deadlock isEqualToString:crashType];
}
...
{
    return [report[TTSDKCrashField_Signal][TTSDKCrashField_Signal] integerValue] == SIGTERM;
}

- (BOOL)isMemoryTermination:(NSDictionary *)report
{
    return [report[TTSDKCrashField_Type] isEqualToString:TTSDKCrashExcType_MemoryTermination];
}

- (NSString *)diagnoseCrash:(NSDictionary *)report
{
    @try {
        NSString *lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:TTSDKCrashField_SymbolName];
        ...
        NSString *crashType = [errorReport objectForKey:TTSDKCrashField_Type];
        if ([crashType isEqualToString:TTSDKCrashExcType_NSException]) {
            NSDictionary *exception = [errorReport objectForKey:TTSDKCrashField_NSException];
            NSString *name = [exception objectForKey:TTSDKCrashField_Name];
            NSString *reason = [exception objectForKey:TTSDKCrashField_Reason]
                                   ? [exception objectForKey:TTSDKCrashField_Reason]
                                   : [errorReport objectForKey:TTSDKCrashField_Reason];
            return [self
        ...
        TTSDKCrashDoctorFunctionCall *functionCall = [self lastFunctionCall:report];
        ...
        if ([self isInvalidAddress:errorReport]) {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:TTSDKCrashField_Address] unsignedLongLongValue];
            if (address == 0) {
        ...
        if (symbols) {
            return [NSString
                stringWithFormat:
                    @"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the TTSDKCrash project.", e,
                    symbols];
        }
        return [NSString
